...
WHERE countrycode = (SELECT code FROM countries WHERE region = 'Southern Europe' ORDER BY population ASC LIMIT 1)
AND isofficial = 'true';
...
WHERE code IN (
    ...
    WHERE language = (SELECT language FROM countrylanguages
                      WHERE countrycode = (SELECT code FROM countries WHERE region = 'Southern Europe' ORDER BY population ASC LIMIT 1)
                      AND isofficial = 'true')
...
WHERE countrycode = (
    ...
    WHERE name = (SELECT name FROM countries
                  WHERE code IN (
                      SELECT countrycode FROM countrylanguages
                      WHERE language = (SELECT language FROM countrylanguages
                                        WHERE countrycode = (SELECT code FROM countries WHERE region = 'Southern Europe' ORDER BY population ASC LIMIT 1)
...
) AND name <> (SELECT name FROM countries WHERE code = (
    SELECT code FROM countries
    WHERE name = (SELECT name FROM countries
                  WHERE code IN (
                      SELECT countrycode FROM countrylanguages
...
WHERE c.name LIKE (SELECT name || '%' FROM cities
                  WHERE countrycode = (
                      SELECT code FROM countries
                      WHERE name = (SELECT name FROM countries
                                    WHERE code IN (
...
SELECT capital
FROM countries
WHERE name = (SELECT country FROM (
    SELECT c.name, co.name AS country
    FROM cities c
    JOIN countries co ON c.countrycode = co.code
    WHERE c.name LIKE (SELECT name || '%' FROM cities
